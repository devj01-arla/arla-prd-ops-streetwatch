  ...
  return (
    <div className="card live-feed-container" onMouseMove={handleMouseMove}>
      ...
      <div className="video-wrapper">
        ...
        <img
          src={FASTAPI_URL ? `${FASTAPI_URL}/stream/video` : NO_SIGNAL_IMAGE}
          alt="Video Stream"
          className="main-stream"
          onError={(event) => {
            event.target.onerror = null;
            event.target.src = NO_SIGNAL_IMAGE;
          }}
        />
        ...
        {lastDetection && (
          <div className="detection-overlay">
            <span className="badge-detection-alert">
              CRUCE: {(lastDetection.class || lastDetection.type || '').toUpperCase()} //{' '}
              {Math.round((lastDetection.confidence || 0) * 100)}%
            </span>
          </div>
        )}
      </div>
